...
app.get('/query', async (req, res) => {
    ...
    if (title) {
        ...
        query = 'MATCH (movie:Movie) WHERE movie.title CONTAINS ($title) RETURN movie.title as title';
        params = { title };
    } else if (genres) {
        // Se corrigió la consulta para que utilice el parámetro $genres y devuelva un objeto con la propiedad 'title'.
        query = 'MATCH (movie:Movie) WHERE any(g in movie.genres WHERE g = $genres) RETURN movie.title as title';
        params = { genres };
    } else {
...
app.get('/movie-details', async (req, res) => {
    ...
    const query = 'MATCH (movie:Movie) WHERE movie.title = $title RETURN movie AS movieDetails';
    const params = { title };
...
app.get('/api/users', async (req, res) => {
    ...
    if (userId) {
        query = 'MATCH (user:User) WHERE user.userId = $userId RETURN user.userId AS user';
        params = { userId };
    } else {
        query = 'MATCH (user:User) RETURN user.userId as id Order by id';
    }

    try {
        const results = await fetchQueryResult(query, params);
        res.json(results.map(record => record.user.properties));
    } catch (error) {
